/*let head = document.getElementById("head").innerHTML=`<meta http-equiv="content-type" content="text/html; charset=utf-8" />
<title>ЗВЕЗДНЫЕ ВОЙНЫ1 BERLIN ANDREW</title>
...
style.innerHTML=`#t {
  ...
  p.N = function(t) {
    ...
    let s, i = [],
      h = 0;t.toLowerCase().split("").forEach(t => {
      let e = hL[t] || [],
        a = h;e.forEach(t => {
        i.push([h + t[0], t[1], h + t[2], t[3]]),
        a = max(a, h + t[0] + .4, h + t[2] + .4)
      }),
      e.length || (a += 1),
      s = (h = a) - .4
    });
    ...
    t.aB.forEach((r, l) => {
      N(() => {
        let _ = t.bI[l],
          n = _u(0, (h - _ * e) / a, 1),
          o = bc((l + _) % 2 - .5);l % 2 - .5 > 0 ? translate(o * (1 - n) * 50, 0) : translate(0, o * (1 - n) * 50),
        R.globalAlpha *= n,
        beginPath(),
        moveTo(r[0] * s, r[1] * i),
        lineTo(r[2] * s, r[3] * i),
  ...
  eI = (() => {
    ...
    let r = e.createRadialGradient(a.width / 2, a.height / 2, t.width / 2, a.width / 2, a.height / 2, t.width / 2 + s);r.addColorStop(0, i),
    r.addColorStop(1, h),
    e.I(r),
    e.fr(0, 0, a.width, a.height);
    try {
      e.drawImage(t, s, s)
    } catch (t) {}
  })), _C = ((t, s, i) => {
    let h;U.cN.push(h = {
      ...
      dV: 1,
      _e: () => N(() => {
        if (i) return i(h);V._x(h, h.hn) && (R.globalAlpha = h.dV, I(h.bi), beginPath(), arc(h.x, h.y, h.hn / 2, 0, M), fill())
      })
    }),
    s.forEach((t, s) => {
      let i = [h].concat(t);s || (i[7] = (() => U._D(U.cN, h))),
      _g.apply(0, i)
    })
  });
  class ae {
  ...
  class hc extends ae {
    Y() {
      V.cR = 1, G._l("Следовать краткому руководству?", [{
        hM: "Yes",
        hX: () => this._r(new ha)
      }, {
        hM: "No",
  ...
  class ha extends ae {
    Y() {
      G._l("Используйте клавиши со стрелками, чтобы управлять своим кораблем");
      let t = {
        x: U.D.x,
        y: U.D.y
      };
  ...
  class gZ extends ae {
    Y() {
      G._l("Связь ОФФЛАЙН.  Найдите ресурсы для ремонта", [{
        hM: "Help",
        hX: () => G._l("Ресурсы можно собирать, уничтожая астероиды")
      }]), this._v(1, () => {
        U.D.B._G >= 20 && this._r(new gW),
        U._b.length < 10 && U.eC()
      })
    }
  }
  class gW extends ae {
    Y() {
      G._l("Общение ОНЛАЙН.  Молодец"), setTimeout(() => this._r(new eT), 5e3)
    }
  }
  class eT extends ae {
    Y() {
      V.cR = 0, U.gU(), G._l("Принесите МИР в галактику или ВОЙНУ...", [{
        hM: di,
        hX: () => this._r()
      }]), G.df = 0
    }
  ...
  class gS extends ae {
    ...
    Y() {
      ...
      this._v(1, s => {
        (t -= s) < 0 && this._r()
      }), G._l(() => "Входящее сообщение от" + this.O.B.K.dR() + " - " + da(t), [{
        hM: "Отвечать",
        hX: () => {
          t = 15,
          G._l(() => this.O._M + " - " + da(t), [{
    ...
    _r(t) {
      super._r(t), t || (this.O.B.aH(-.1), G._l("Связь игнорируется. " + this.O.B.K._B + " запомнит это", [{
        hM: di,
        hX: () => G._l()
      }]), setTimeout(() => G._l(), 5e3))
    }
  ...
  class gL extends aF {
    constructor(t) {
      super(), this.H = t, this._M = "Уничтожить объекты на " + t.dR(), this._t = [t], this.dP = 0, this.dN = min(this.H.aq.length, ~~C(3, 5))
    }
    ag() {
      return "Объекты уничтожены: " + this.dP + "/" + this.dN
    }
    Y() {
      super.Y(), this._v(0, t => {
  ...
  class gI extends aF {
    constructor(t) {
      super(), this.J = t, this._M = "Собрать некоторые данные о " + t.dR() + " для нас", this._t = [t], this.dG = 0
    }
    ag() {
      return this.dF() ? "Сбор данных... - " + ~~(100 * this.dG) + "%" : "Подойди ближе " + this.J._B
    }
    Y() {
      super.Y(), this._v(1, t => {
  ...
  class gG extends aF {
    constructor() {
      super(), this._M = "Собери для нас немного ресурсов", this.bl = 100, this.gF = 0
    }
    ag() {
      return "Собирайте ресурсы - " + this.gF + "/" + this.bl
    }
    Y() {
      super.Y(), this._v(3, () => {
        U.D.B._G--,
        ++this.gF >= this.bl && this.dZ(this.B.K, "Вернуться к " + this.B.K._B)
      })
    }
  }
  class gE extends aF {
    constructor() {
      super(), this._M = "Уничтожьте эти астероиды для нас"
    }
    ag() {
      return "Уничтожить астероиды"
    }
    Y() {
      super.Y();
      let t = [];
      ...
      this._v(1, () => {
        this._t = t.filter(t => t._k > 0),
        this._t.length || G.aY(1)
      })
    }
  }
  class gz extends aF {
    constructor() {
      super(), this._M = "Помогите нам бороться с этими пиратами..."
    }
    ag() {
      return "Уничтожить пиратские корабли"
    }
    Y() {
      super.Y();
    ...
    _e() {
      ...
      N(() => {
        ...
        for (let s = 0; s < 400; s++) N(() => {
          let s = t._f(.1, .3);translate(cZ(1e3 * t._f(-1, 1) - U.D.x * s, 1e3), cZ(t._f(-1, 1) * __ - U.D.y * s, __)),
          scale(20 * s, 20 * s),
          beginPath(),
          arc(0, 0, 1, 0, M),
          fill()
        ...
        scale(V.ac, V.ac),
        translate(-V.x, -V.y),
        this.forEach([this.aA, this.cN, this._p, this._b, this.aV], t => N(() => {
          t._e()
        }))
      })
    }
  ...
  class gf extends cw {
    constructor(t, s, i) {
      ...
      this._w = cT(() => eH(aU(2 * this.$, 2 * this.$, t => {
        t.I("#fff"),
        t.arc(this.$, this.$, this.$, 0, M),
        t.fill(),
        t.globalCompositeOperation = "source-atop";
        let s = bW(this._B.charCodeAt(0) + this.$),
          i = [...Array(3)].map(() => s._f(32, 255));
        for (let h = 0; h < 2 * this.$; h += s._f(10, 20)) t.I("rgb(" + i[0] + "," + i[1] + "," + i[2] + ")"),
        t.fillRect(0, h, 2 * this.$, 2 * this.$),
        i = i.map(t => ~~_u(32, t + s._f(-20, 20), 255))
      }), 50, "rgba(255,255,255,0.1)", "rgba(255,255,255,0)")), this.dX = cT(() => aU(4 * this.$, 2 * this.$, (t, s) => {
        let i = t.createLinearGradient(0, 0, s.width, 0);i.addColorStop(0, "#000"),
        ...
        t.N(() => {
          t.translate(0, this.$),
          t.scale(.7, 1),
          t.arc(0, 0, this.$, -PI / 2, PI / 2)
        }),
        t.lineTo(s.width, s.height),
        t.lineTo(s.width, 0),
        t.fill()
      }))
    }
    ...
    _e() {
      V._x(this._N, this._R + 2 * this.$) && (R.lineWidth = 10, R.strokeStyle = "rgba(255,255,255,0.1)", beginPath(), arc(this._N.x, this._N.y, this._R, 0, M), stroke(), V._x(this, this.$ + 50) && (R.strokeStyle = "#fff", R.lineWidth = 60, this.eb && N(() => {
        ...
        rotate(this.A),
        drawImage(this._w(), -this._w().width / 2, -this._w().height / 2)
      }), N(() => {
        rotate(this.bB),
        drawImage(this.dX(), 0, -this.dX().height / 2)
      }), this.eb && N(() => {
        R.globalAlpha = .5,
        rotate(PI / 4),
        scale(1, .2),
        beginPath(),
        arc(0, 0, 1.5 * this.$, -PI, 0),
        stroke()
      }), this.eq()))
    }
    dR() {
  ...
  onresize = (() => {
    var t, s, i = innerWidth,
      ...
      e = i / h,
      a = 1e3 / __,
      r = document.querySelector("#cc").style;e <= a ? s = (t = i) / a : t = (s = h) * a,
    r.width = t + "px",
    r.height = s + "px"
  ...
  fd = (t => t._a(fb) + t._a(fc) + t._a(fb.concat(fc))), el = (t => fd(t) + fd(t) + " " + t._a(["III", "IV", "V", "VI", "VII", "IX", "X"])), fe = (t => {
    ...
    h.forEach((t, s) => {
      t.forEach((i, e) => {
        let a = t[(e + 1) % t.length];i.aS.push(a),
        a.aS.push(i);
        let r = h[(s + 1) % h.length][e];i.aS.push(r),
        r.aS.push(i)
      })
    }),
    i
    ...
    let s = new Uint32Array([imul(t, 2246822507), imul(t, 3266489909)]),
      i = () => {
        let t = s[0],
          i = s[1] ^ t;
        return s[0] = (t << 26 | t >> 8) ^ i ^ i << 9,
        s[1] = i << 13 | i >> 19,
        (imul(t, 2654435771) >>> 0) / 4294967295
      };
    return {
      _a: t => t[~~(i() * t.length)],
      _f: (t, s) => i() * (s - t) + t,
  ...
  w.Z = {}, onkeydown = (t => {
    ...
    w.Z[t.keyCode] = 0;
    let s = String.fromCharCode(t.keyCode).toLowerCase();isNaN(s) && G.e_(s)
  }), ontouchstart = (t => ontouchmove(t, 1)), ontouchmove = ontouchend = ((t, s) => {
    t.preventDefault();
    let i = document.querySelector("canvas").getBoundingClientRect();w.Z[32] = 0,
    w.Z[37] = 0,
    w.Z[38] = 0,
    w.Z[39] = 0;
  ...
  class hw {
    ...
    cC(t, s, i, h, e) {
      N(() => {
        translate(t, s),
        I("rgba(128,128,128,0.5)"),
        fr(0, -5, 200, 10),
        I(h),
        fr(0, -5, -2, 10),
        fr(200, -5, 2, 10),
        fr(0, -5, 200 * _u(0, i, 1), 10),
        translate(-25, 0),
        e()
      })
    }
    ...
    W(t) {
      G.S += t, G.bP && ((G.df -= t) <= 0 && G.hC(), U.W(t), G.aJ.c_(1, t)), ca.slice().forEach(s => s.W(t)), w.Z[13] && G.bg(), U._e(), N(() => {
        ...
        let t = U._b.reduce((t, s) => (s.B && (t[s.B._y()]++, t.total++), t), {
          total: 1,
          "#0f0": 0
        });fillText("Мир: " + ~~(100 * t["#0f0"] / t.total) + "%", 185, 140),
        G.cC(100, 50, U.D._k, U.D._k < .25 || G.S - U.D.bb < .2 ? "#f00" : G.ct, () => {
          scale(.5 * G.ff, .5 * G.ff),
          beginPath(),
          moveTo(0, -15),
          lineTo(14, -10),
        ...
        N(() => {
          ...
          R.shadowBlur = 10,
          I("cyan"),
          fr(100, 45, 200 * _u(0, U.D.aj, 1), 10)
        }),
        ...
        let s = [],
          ...
          h = i[0] && F(i[0], U.D) < i[0].ai;h && !i[0].hI && (i[0].hI = 1, G.dE("обнаружена система - " + i[0]._B)),
        G.O ? s = G.O._t || [] : h || (s = i),
        s.forEach(t => {
          if (F(t, U.D) < (t.ai || 0)) return;
          let s = _Q(U.D, t);N(() => {
            let i = 200 * _u(0, (F(t, U.D) - t.ai) / 4e3, 1) + 50;translate(500 + cos(s) * i, __ / 2 + sin(s) * i),
            rotate(s),
            I(G.O ? "#f80" : "#888"),
            beginPath(),
            moveTo(0, 0),
        ...
        let e = G._$();e && N(() => {
          I("rgba(0,0,0,0.5)"),
          R.font = "20pt " + au,
          translate(0, __ - (bn ? 400 : 200)),
          fr(0, 0, 1e3, 200);
          let t = measureText(e + "_").width,
            s = this.f$();I("#fff"),
          R.textAlign = "left",
          G.aZ || fillText(s, (1e3 - t) / 2, 50),
          s.length >= e.length && (R.textAlign = "center", R.textBaseline = "middle", R.font = "16pt " + au, G.bH.forEach((t, s) => {
            if (I("#fff"), G.aZ) {
              if (G.aZ != t) return;
              R.globalAlpha = (sin(G.S * M * 4) + 1) / 2
            }
            let i = "[" + t.hM[0] + "]" + t.hM.slice(1),
              h = (s + 1) * (1e3 / (G.bH.length + 1));fr(h - 75, 85, 150, 30),
            R.globalAlpha = 1,
            I("#000"),
            fillText(i, h, 100)
          }))
        });
        let a = U.D._F();a && a != G._F && (G.hN = G.S + 3, G._F = a, hm()),
        a || (G._F = 0),
        G._F && G.S < G.hN && (I("rgba(255,0,0,0.5)"), fr(0, 200, 1e3, 125), R.font = "36pt " + au, R.textBaseline = "middle", R.textAlign = "center", I("#fff"), fillText("/!\\ WARNING /!\\", 500, 250), R.font = "18pt " + au, fillText(G._F, 500, 300), G._Z = 0),
        R.strokeStyle = "#fff",
        R.lineCap = "round",
        G._Z && G.hO && N(() => {
          R.lineWidth = 4;
          let t = 20 * G._Z.width;translate((1e3 - t) / 2, (__ - 100) / 2 - 200),
          aQ(G._Z, 20, 30, G.hO, .1, 1)
        }),
        N(() => {
          translate(0, G.dW),
          I("#000"),
          fr(0, 0, 1e3, __),
          R.lineWidth = 8;
          let t = (__ - 12 * G.cf / 5) / 2;N(() => {
            translate((1e3 - G.bZ.width * G.eU) / 2, t),
            aQ(G.bZ, G.eU, G.cf, G.S - .5, .1, 1)
          }),
          N(() => {
            R.lineWidth = G.hR,
            translate((1e3 - G.c$.width * G.cg) / 2, t + 7 * G.cf / 5),
            aQ(G.c$, G.cg, G.eY, G.S - .5, G.bZ.aB.length / G.c$.aB.length * .1, 1)
          }),
          R.lineWidth = 4;N(() => {
        ...
        bn && (translate(0, __ - 200), R.globalAlpha = .8, I("#000"), fr(0, 0, 1e3, 200), I("#fff"), translate(0, 100), N(() => {
          R.globalAlpha = w.Z[37] ? 1 : .5,
          translate(125, 0),
          rotate(PI),
          G.d_()
        }), N(() => {
          R.globalAlpha = w.Z[39] ? 1 : .5,
          translate(375, 0),
          G.d_()
        }), N(() => {
          R.globalAlpha = w.Z[32] ? 1 : .5,
          translate(625, 0),
          beginPath(),
          moveTo(0, 0),
          arc(0, 0, 40, 0, M),
    ...
    aY(t) {
      let s = G.O;
      G.bQ(), s.B.aH(t ? .5 : -.2), G._l("Миссия " + (t ? "УСПЕХ" : "НЕ УДАЛОСЬ") + ". " + s.B.K._B + " это запомнит.", [{
        hM: di,
        hX: () => G._l()
      }])
    }
    ...
    fK() {
      ...
      t = i + h < 10 ? "тебя едва заметили" : abs(i - h) < 5 ? "мало что изменилось" : i > h ? "ты принес войну" : "ты принес мир", G.bZ = _E("game over"), G.c$ = _E(t), G.cm = _E("нажмите Enter, чтобы отправить другой корабль"), G.cg = 25, G.eY = 50, G.hR = 6, G.dl = G.bP = 0, G.S = 0, _g(G, "dW", -__, 0, .3, 0, 0, () => G.eR()), setTimeout(() => {
        G.fo = [i + " планеты объявили вам войну", h + " виды стали вашими союзниками"],
        G.dl = 1
      }, 4e3)
    }
    eR() {
  ...
  onload = (() => {
    onresize();
    let t = document.querySelector("canvas");t.width = 1e3,
    t.height = __,
    R = t.getContext("2d"),
    Object.getOwnPropertyNames(p).forEach(t => {
